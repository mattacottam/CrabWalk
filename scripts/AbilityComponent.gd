...
func _init(owner_unit: Unit):
	...
	if unit.character_data:
		ability_name = unit.character_data.ability_name
		ability_description = unit.character_data.ability_description
		ability_damage = unit.character_data.ability_damage
		ability_mana_cost = unit.character_data.ability_mana_cost
		mana = unit.current_mana  # Start with unit's current mana
...
func end_combat():
	...
	if ability_range_indicator:
		ability_range_indicator.queue_free()
		ability_range_indicator = null

# Called each combat tick
func update_combat(delta: float):
	...
	if cooldown_remaining <= 0 and not is_casting:
		...
		# Cap mana at max
		if unit.character_data:
			mana = min(mana, unit.character_data.mana_max)
		
		# Update the unit's mana display
		unit.current_mana = mana
		if unit.health_bar_system:
			unit.health_bar_system.update_mana_display(unit.current_mana)
...
func start_casting():
	...
	print(unit.character_data.display_name + " is casting " + ability_name)
...
func finish_casting():
	...
	print(unit.character_data.display_name + " used " + ability_name)
...
func create_casting_effect():
	...
	if unit.character_data:
		var base_color = unit.character_data.color
...
func berserker_ability():
	...
	if unit.character_data:
		...
		health_percent = 1.0 - (current_health / float(max_health))
	else:
	...
	apply_attack_speed_buff(unit, attack_speed_buff, 5.0)  # 5 second buff
...
func get_nearby_allies(radius: float) -> Array:
	...
	var ally_list = combat_system.player_units if not unit.character_data.is_enemy else combat_system.enemy_units
	
	for ally in ally_list:
		if ally == unit:  # Skip self
			continue
...
func get_nearby_enemies(radius: float) -> Array:
	...
	var enemy_list = combat_system.enemy_units if not unit.character_data.is_enemy else combat_system.player_units
...
func get_nearest_enemy(max_range: float):
	...
	var enemy_list = combat_system.enemy_units if not unit.character_data.is_enemy else combat_system.player_units
	var nearest = null
	var nearest_distance = max_range + 1.0  # Start beyond max range
...
func get_furthest_enemy(max_range: float):
	...
	for enemy in enemy_list:
		var distance = unit.global_position.distance_to(enemy.global_position)
		if distance <= max_range and distance > furthest_distance:
			furthest = enemy
...
func get_lowest_health_enemy(max_range: float):
	...
	for enemy in enemy_list:
		var distance = unit.global_position.distance_to(enemy.global_position)
		if distance <= max_range and enemy.current_health < lowest_health:
...
func get_lowest_health_ally(max_range: float):
	...
	for ally in ally_list:
		...
		var distance = unit.global_position.distance_to(ally.global_position)
		if distance <= max_range and ally.current_health < lowest_health:
...
func deal_damage(target, amount: int):
	...
	# Show damage text
	show_damage_text(target.global_position, amount)
...
func heal_target(target, amount: int):
	...
	# Show healing text (using green color)
	show_healing_text(target.global_position, amount)
...
func apply_armor_buff(target, amount: int, duration: float):
	...
	# Here you would add an armor buff status effect to the target
	# Since this isn't implemented yet, we'll just print for now
	print("Applied +" + str(amount) + " armor for " + str(duration) + " seconds to " + target.character_data.display_name)
	
	# Show buff text
	show_buff_text(target.global_position, "+" + str(amount) + " Armor")
...
func apply_attack_speed_buff(target, percent: float, duration: float):
	...
	# Here you would add an attack speed buff status effect to the target
	# Since this isn't implemented yet, we'll just print for now
	print("Applied +" + str(percent) + "% attack speed for " + str(duration) + " seconds to " + target.character_data.display_name)
	
	# Show buff text
	show_buff_text(target.global_position, "+" + str(int(percent)) + "% Speed")
...
func apply_shield(target, amount: int, duration: float):
	...
	# Here you would add a shield status effect to the target
	# Since this isn't implemented yet, we'll just print for now
	print("Applied " + str(amount) + " damage shield for " + str(duration) + " seconds to " + target.character_data.display_name)
	
	# Show shield text
	show_buff_text(target.global_position, str(amount) + " Shield")
...
func taunt_enemy(enemy, duration: float):
	...
	# Here you would force the enemy to target this unit
	# Since this isn't implemented yet, we'll just print for now
	print("Taunted " + enemy.character_data.display_name + " for " + str(duration) + " seconds")
	
	# Show taunt text
	show_buff_text(enemy.global_position, "Taunted!")
...
func immobilize_target(target, duration: float):
	...
	# Here you would prevent the target from moving
	# Since this isn't implemented yet, we'll just print for now
	print("Immobilized " + target.character_data.display_name + " for " + str(duration) + " seconds")
	
	# Show immobilize text
	show_buff_text(target.global_position, "Rooted!")
...
func create_projectile_effect(target_pos: Vector3, color: Color, duration: float = 0.5):
	...
	projectile.position = unit.global_position + Vector3(0, 1.5, 0)
...
func create_teleport_effect(target_pos: Vector3):
	...
	disappear.position = unit.position
	board.add_child(disappear)
...
func show_damage_text(pos, amount):
	...
	text.font_size = 64
	text.modulate = Color(1, 0.3, 0.3)  # Red for damage
	text.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	
	# Position above target unit
...
# Show healing text
func show_healing_text(pos, amount):
	# Create 3D text to show healing
	var text = Label3D.new()
	...
	text.text = "+" + str(amount)
	text.font_size = 64
	text.modulate = Color(0.3, 1, 0.3)  # Green for healing
	text.billboard = BaseMaterial3D.BILLBOARD_ENABLED
...
# Show buff text
func show_buff_text(pos, buff_text):
	# Create 3D text to show buff
	var text = Label3D.new()
	...
	text.text = buff_text
	text.font_size = 48
	text.modulate = Color(0.3, 0.3, 1.0)  # Blue for buffs
	text.billboard = BaseMaterial3D.BILLBOARD_ENABLED
